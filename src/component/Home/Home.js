...
export default function Home(props) {
  ...
  async function addHandle(id) {
    ...
    if (role === "guest") {
      ...
      const a = await axios.post(
        process.env.REACT_APP_URL + "tournament/my",
        { id: id },
        {
  ...
  useEffect(() => {
    async function tournament() {
      const a = await axios.get(process.env.REACT_APP_URL + "tournament");
      setlist(a.data);
    }
    tournament();
    cnt = 0;
      ...
      <div className="container">
        <Row gutter={[0, 16]} style={{ width: "100%" }}>
          <Col style={{ textAlign: "center" }} span={24}>
            <img
              src="https://cdn.cloudflare.steamstatic.com/apps/dota2/images/blog/play/dota_logo.png"
              alt="Dota 2 Logo"
            />
              ...
              <Col key={x.id} span={18} offset={3}>
                <Row style={{ color: t, backgroundColor: c }}>
                  ...
                  <Col span={14}>
                    ...
                    <footer>
                      Sites :{" "}
                      <a href={x.contact} style={{ color: "rgb(192, 75, 46)" }}>
                        {x.contact}
                      </a>
                    </footer>
                  </Col>
                  <Col span={4}>
                    ...
                    >
                      <Button
                        type="primary"
                        style={{
                          backgroundColor: "rgb(136, 130, 126)",
                          color: "#0A0A0A",
                        }}
                        onClick={() => addHandle(x.id)}
                      >
                        + Watch List
                      </Button>
                    </Row>
